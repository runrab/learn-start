...
public class ParamUtil {
    ...
    public static final Date checkDate(String dateString) {
        ...
        try {
            ...
            timeString.add(fill(RandomNum.getRandInt(0,23),2));
            timeString.add(fill(RandomNum.getRandInt(0,59),2));
            timeString.add(fill(RandomNum.getRandInt(0,59),2));
            ...
            throw new RuntimeException("必须为日期型格式 例如： 2020-11-12");
        }
    }

    public static String fill(Object obj,int length){
        String str = obj.toString();
        int tmp = length - str.length();
        for (int i = tmp; i >0 ; i--) str = "0"+str;
        return str;
    }
